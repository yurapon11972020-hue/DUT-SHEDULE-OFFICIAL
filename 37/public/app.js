...
function updateSupportUnreadBadge() {
  const toggle = document.querySelector('.support-chat-toggle');
  if (!toggle) return;
  const oldBadge = toggle.querySelector('em');
...
function updateSupportMessagesOnly({ forceScroll = false } = {}) {
  const body = document.getElementById('supportChatBody');
  if (body) {
    const nearBottom = body.scrollHeight - body.scrollTop - body.clientHeight < 80;
    body.innerHTML = supportThreadMessages();
    if (forceScroll || nearBottom) body.scrollTop = body.scrollHeight;
  }
  const headStatus = document.querySelector('.support-chat-head small');
  if (headStatus) {
    const thread = state.supportThread;
    headStatus.textContent = thread?.status === 'answered'
      ? tr('Есть ответ администратора')
      : thread?.status === 'closed'
        ? tr('Обращение закрыто')
        : tr('Поддержка онлайн');
  }
...
async function refreshSupportThread(options = {}) {
  ...
  try {
    ...
    const previousIds = (state.supportThread?.messages || []).map((message) => message.id).join('|');
    state.supportThread = data.thread || null;
    const currentIds = (state.supportThread?.messages || []).map((message) => message.id).join('|');
    state.supportUnread = state.supportOpen ? 0 : computeSupportUnread(state.supportThread);
    if (state.supportOpen) markSupportRead();
    if (silent && state.supportOpen) {
      updateSupportMessagesOnly({ forceScroll: previousIds !== currentIds });
      return;
    }
  } catch (error) {
    ...
    state.supportLoading = false;
    if (silent && state.supportOpen) return;
    updateSupportWidget();
    scrollSupportToBottom();
  }
}
...
function syncColumnDomAfterOptimisticMove(taskId, previousColumnId, nextColumnId) {
  const ids = [previousColumnId, nextColumnId].filter(Boolean);
  ids.forEach((columnId) => {
    const column = document.querySelector(`.column[data-column-id="${cssEscape(columnId)}"]`);
    const list = column?.querySelector('.task-list');
    if (!column || !list) return;
    const visibleCards = Array.from(list.querySelectorAll('.task-card'));
    const empty = list.querySelector('.empty-state');
    const addButton = list.querySelector('.add-task-zone');
    const columnTasks = state.tasks.filter((task) => task.columnId === columnId);
    const visibleTasks = tasksForColumn(columnId);
    if (visibleCards.length || visibleTasks.length) {
      if (empty) empty.remove();
    } else if (!empty && addButton) {
      addButton.insertAdjacentHTML('beforebegin', `<div class="empty-state"><strong>${escapeHtml(tr(columnTasks.length ? 'По текущему фильтру задач нет' : 'Здесь пока нет задач'))}</strong><p>${escapeHtml(tr(columnTasks.length ? 'Измени поиск или сбрось фильтр, чтобы увидеть карточки.' : 'Создай первую задачу или перенеси карточку из другой колонки.'))}</p></div>`);
    }
    const counter = column.querySelector('.task-counter');
    if (counter) counter.textContent = `${visibleTasks.length}/${columnTasks.length}`;
  });
}
...
async function saveTaskMove(taskId, columnId) {
  ...
  syncColumnDomAfterOptimisticMove(taskId, previousColumnId, columnId);
  document.body.classList.remove('task-drag-active');
  document.querySelectorAll('.column.drop-active').forEach((item) => item.classList.remove('drop-active'));
