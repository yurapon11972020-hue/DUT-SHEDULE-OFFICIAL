...
function renderShell(content) {
  root.innerHTML = `
    <header class="topbar liquid-panel">
      <div class="brand">
        <div class="logo-box"><img src="/dut-logo-light.png" alt="DUT"></div>
        <div class="brand-title"><strong>DUT Admin</strong></div>
      </div>
...
function startAdminPolling() {
  ...
  adminPollTimer = setInterval(async () => {
    if (document.hidden || !state.admin) return;
    try {
      if (state.tab === 'requests') await refreshRequests();
      else if (state.tab === 'users') await refreshUsers();
      else if (state.tab === 'audit') await refreshStats();
      render();
    } catch {}
  }, 9000);
}
