...
let state = { user: null, board: null, columns: [], tasks: [], message: '', error: '', dragTaskId: null, filters: { taskQuery: '', status: 'all' } };
...
function applyVisualSettings(settings = {}) {
  ...
  const root = document.documentElement;
  const datasetKeys = ['theme','background','font','textSize','density','radius','shadows','surface','borders','buttonStyle','inputStyle','taskStyle','taskSize','columnStyle','columnHeader','boardWidth','boardGap','columnHeight','hoverEffect','animations','headerStyle','controlSize','cardTone','contrast','glassPower','accentMode','columnColorMode'];
...
const liquidGlassSelector = [
  ...
  '.task-color-meta', '.task-color-swatch', '.add-task-zone', '.icon-btn', '.site-footer', '.nav-link', '.feature-card', '.process-step', '.faq-card', '.landing-cta', '.board-toolbar', '.empty-state', '.info-page', '.contact-card', '.legal-card', '.method-card', '.sitemap-card'
].join(',');
...
function layout(content, options = {}) {
  return `
    ...
    <main class="page-shell" id="content" tabindex="-1">
      <header class="topbar">
        ${brand()}
        <nav class="nav primary-nav" aria-label="${escapeHtml(tr('Основная навигация'))}">
          ${renderTopNav()}
        </nav>
      </header>
      ${options.breadcrumbs || ''}
...
function renderTask(task) {
  ...
  return `
    <article class="${cls}" ${manualStyle} draggable="true" data-task-id="${escapeHtml(task.id)}">
      ...
      <div class="card-actions">
        <button class="btn tiny secondary edit-task" data-task-id="${escapeHtml(task.id)}">${tr('Изменить')}</button>
        <button class="btn tiny danger delete-task" data-task-id="${escapeHtml(task.id)}">${tr('Удалить')}</button>
      </div>
    </article>
  `;
}
...
function bindBoardEvents(plan, customColumnsCount) {
  ...
  document.querySelectorAll('.task-card[draggable="true"]').forEach((card) => {
    card.addEventListener('dragstart', () => { state.dragTaskId = card.dataset.taskId; });
    card.addEventListener('dragend', () => { state.dragTaskId = null; });
  });
  document.querySelectorAll('.column').forEach((column) => {
    column.addEventListener('dragover', (event) => event.preventDefault());
    column.addEventListener('drop', async () => {
      if (!state.dragTaskId) return;
      await saveTaskMove(state.dragTaskId, column.dataset.columnId);
...
function openColumnModal(column = {}) {
  ...
  modal.className = 'modal-backdrop';
  modal.innerHTML = `
    <form class="modal" id="columnForm">
...
async function saveTaskMove(taskId, columnId) {
  try { await api(`/api/tasks/${encodeURIComponent(taskId)}`, { method: 'PATCH', body: JSON.stringify({ columnId }) }); await renderDashboard(); }
  catch (error) { state.error = error.message; await renderDashboard(); }
}
...
function bindDesignSettings() {
  const form = document.getElementById('designForm');
  if (!form) return;
  const applyCurrent = () => { applyVisualSettings(readDesignForm()); updateDesignUiState(); };
  ...
  document.getElementById('randomDesignBtn')?.addEventListener('click', () => {
    ...
    const randomSettings = {
      ...
      accent: ['#2563eb','#7c3aed','#06b6d4','#10b981','#f97316','#ef4444','#111827'][Math.floor(Math.random()*7)]
    };
  ...
  app.innerHTML = layout(`
    <section class="info-page contact-page">
      ...
      <form class="contact-card" id="contactForm">
        ...
        <div class="form-two"><div class="form-group"><label>${escapeHtml(tr('Ваше имя'))}</label><input class="input" name="name" required /></div><div class="form-group"><label>${escapeHtml(tr('Email'))}</label><input class="input" name="email" type="email" required /></div></div>
...
function bindLogout() {
  document.getElementById('logoutBtn')?.addEventListener('click', async () => {
    ...
    state = { user: null, board: null, columns: [], tasks: [], message: '', error: '', dragTaskId: null, filters: { taskQuery: '', status: 'all' } };
