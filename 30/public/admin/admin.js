...
function renderRequest(item) {
  ...
  return `<article class="list-item" data-request-id="${escapeHtml(item.id)}">
    <div class="list-item-head"><div><strong>${escapeHtml(item.topic || 'Без темы')}</strong><div class="muted">${escapeHtml(item.name)} · ${escapeHtml(item.email)} · ${formatDate(item.createdAt)}</div></div><span class="badge ${closed ? 'green' : 'orange'}">${closed ? 'Закрыто' : 'Новое'}</span></div>
    <p>${escapeHtml(item.message || '')}</p>
    <div class="action-row"><button class="btn small ${closed ? 'ghost' : 'ok'}" data-action="request-status" data-status="${closed ? 'open' : 'closed'}">${closed ? 'Вернуть в новые' : 'Закрыть'}</button><button class="btn small danger" data-action="request-delete">Удалить</button></div>
  </article>`;
}
