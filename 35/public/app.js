...
let state = { user: null, board: null, columns: [], tasks: [], message: '', error: '', dragTaskId: null, filters: { taskQuery: '', status: 'all' }, supportOpen: false, supportThread: null, supportLoading: false, supportError: '' };
...
document.addEventListener('click', (event) => {
  ...
  const supportOpen = event.target.closest('[data-support-open]');
  if (supportOpen) {
    event.preventDefault();
    openSupportChat();
    return;
  ...
  const supportRefresh = event.target.closest('[data-support-refresh]');
  if (supportRefresh) {
    event.preventDefault();
...
function renderSupportWidgetContent() {
  ...
  return `
    <div class="support-chat ${state.supportOpen ? 'open' : ''}" aria-live="polite">
      <button class="support-chat-toggle" type="button" data-support-open aria-expanded="${state.supportOpen ? 'true' : 'false'}">
        <span>?</span><b>${escapeHtml(tr('Поддержка'))}</b>
      </button>
      ${state.supportOpen ? `<section class="support-chat-panel" role="dialog" aria-label="${escapeHtml(tr('Чат с администратором'))}">
        <header class="support-chat-head">
          ...
          <div class="support-head-actions"><button type="button" data-support-refresh title="${escapeHtml(tr('Обновить'))}">↻</button><button type="button" data-support-close title="${escapeHtml(tr('Закрыть'))}">×</button></div>
        </header>
...
async function openSupportChat() {
  ...
  state.supportOpen = true;
  updateSupportWidget();
  if (!state.supportThread) await refreshSupportThread();
}
...
function bindLogout() {
  document.getElementById('logoutBtn')?.addEventListener('click', async () => {
    ...
    state = { user: null, board: null, columns: [], tasks: [], message: '', error: '', dragTaskId: null, filters: { taskQuery: '', status: 'all' }, supportOpen: false, supportThread: null, supportLoading: false, supportError: '' };
